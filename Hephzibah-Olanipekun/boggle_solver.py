...
class Boggle:
    ...
    def _build_prefixes(self):
        ...
        for word in self.dictionary:
            # word is already uppercase and len >= 3 from set_dictionary
    ...
    def set_dictionary(self, dictionary):
        ...
        norm_set = set()
        for w in dictionary: #w stands for word
            if isinstance(w, str):
                w_up = w.strip().upper()
    ...
    def get_solution(self):
        ...
        for r in range(self._n): #r for row
            for c in range(self._n): #c for column
                self._dfs_trie_node(
                    r,
                    c,
                    self._trie_root,
...
def _expect_equal(name, actual, expected):
    ...
    Arguments:
        ...
        actual (list[str]): Actual results from get_solution().
...
def run_edge_case_tests():
    ...
    _expect_equal("Empty grid", game.get_solution(), [])
    ...
    dictionary = ["ABC", "DEF"]
    game = Boggle(grid, dictionary)
    _expect_equal("Non-square grid", game.get_solution(), [])
    ...
    dictionary = ["ACD"]
    game = Boggle(grid, dictionary)
    _expect_equal("Non-string tile in grid", game.get_solution(), [])
    ...
    dictionary = ["ACD"]
    game = Boggle(grid, dictionary)
    _expect_equal("Empty tile in grid", game.get_solution(), [])
    ...
    dictionary = ["ACD"]
    game = Boggle(grid, dictionary)
    _expect_equal("Non-alphabetic tile in grid", game.get_solution(), [])
    ...
    dictionary = []
    game = Boggle(grid, dictionary)
    _expect_equal("Empty dictionary", game.get_solution(), [])
    ...
    game = Boggle(grid, dictionary)
    _expect_equal("Dictionary only short/invalid", game.get_solution(), [])
    ...
    game = Boggle(grid, dictionary)
    _expect_equal("Normalization & min length", game.get_solution(), ["ACB"])
    ...
    game = Boggle(grid, dictionary)
    _expect_equal("Diagonal adjacency", game.get_solution(), ["CAT"])
    ...
    dictionary = ["ABA"]
    game = Boggle(grid, dictionary)
    _expect_equal("No tile reuse", game.get_solution(), [])
    ...
    dictionary = ["QUA", "QU"]
    game = Boggle(grid, dictionary)
    _expect_equal("Multi-letter QU", game.get_solution(), ["QUA"])
    ...
    dictionary = ["STAR", "STA"]
    game = Boggle(grid, dictionary)
    _expect_equal("Multi-letter ST", game.get_solution(), ["STA", "STAR"])
    ...
    dictionary = ["TIE", "TI"]
    game = Boggle(grid, dictionary)
    _expect_equal("Multi-letter IE", game.get_solution(), ["TIE"])
    ...
    game = Boggle(grid, dictionary)
    _expect_equal("No matches case", game.get_solution(), [])
...
def main():
    ...
    # <-- use get_solution(), do NOT call .solution()
